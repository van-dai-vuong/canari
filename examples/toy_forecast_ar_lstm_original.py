...
trend_true_norm = trend_true / (
    data_processor.norm_const_std[output_col].item() + 1e-10
)
level_true_norm = (5.0 - data_processor.norm_const_mean[output_col].item()) / (
    data_processor.norm_const_std[output_col].item() + 1e-10
)
...
print(
    "phi_AR =",
    model_dict["states_optimal"].mu_prior[-1][model_dict["phi_index"]].item(),
)
print(
    "sigma_AR =",
    np.sqrt(
        model_dict["states_optimal"].mu_prior[-1][model_dict["W2bar_index"]].item()
    ),
)
pretrained_model = Model(
    ...
    Autoregression(
        ...
        var_states=[
            model_dict["var_states"][
                model_dict["autoregression_index"], model_dict["autoregression_index"]
            ].item()
        ],
    ),
)
