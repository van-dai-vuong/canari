...
# sigma_v_fix = 0.13687003237234974
# look_back_len_fix = 24
# SKF_std_transition_error_fix = 8.546047931523877e-05
# SKF_norm_to_abnorm_prob_fix = 0.0001437092709171301

# Fix parameters
sigma_v_fix = 0.10772637922656891
look_back_len_fix = 24
SKF_std_transition_error_fix = 0.00010424355827940114
SKF_norm_to_abnorm_prob_fix = 3.107657547946687e-05
...
def main(
    ...
    # Read data
    data_file = "./data/benchmark_data/test_14.csv"
    df_raw = pd.read_csv(data_file, skiprows=1, delimiter=";", header=None)
    time_series = pd.to_datetime(df_raw.iloc[:, 4])
    df_raw = df_raw.iloc[:, 6:8]
    df_raw.index = time_series
    df_raw.index.name = "date_time"
    df_raw.columns = ["piezometrique", "water_level"]
...
def training(model, data_processor, num_epoch: int = 100):
    ...
    # model.auto_initialize_baseline_states(data_processor.train_split["y"][0:104])
    model.auto_initialize_baseline_states(data_processor.train_split["y"][0:60])
